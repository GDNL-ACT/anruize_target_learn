...
from genRep_encode import genRepEncode
...
os.environ["HF_HOME"] = "/home/anruize24/.cache/huggingface/"
...
def main():
    ...
    encode = genRepEncode(
        model = genRep_model,
        tokenizer = genRep_model.tokenizer
    )
    
    print("here: ", args.use_instructions)
    model = genRepWrapper(model = encode, 
                          mode = args.mode,
    ...
    results = evaluation.run(model, output_folder=args.output_dir, 
                             encode_kwargs={"batch_size": args.batch_size}, task_config="default")
